...
const PickupParcel = () => {
  ...
  const [deliveryPincodeSuggestions, setDeliveryPincodeSuggestions] =
    useState("");
  ...
  useEffect(() => {
    const fetchData = (inputValue) => {
      try {
        const data = require("../../../pincodes/pincode.json").concat(
          maharastraData,
          goa_gujratData,
          jammu_kashmir,
          north_east
  ...
  const getExpectedDeliveryTime = (deliveryPincode, pickupType) => {
    ...
    let region = isMaharastraPincode(deliveryPincode)
      ? "maharashtra"
      : "default" && isgoa_gujratPincode(deliveryPincode)
      ? "goa_gujrat"
      : "default" && isallIndia(deliveryPincode)
      ? "AllIndia"
      : "default" && isj_kpin(deliveryPincode)
      ? "jammu_kash"
      : "default" && isnoth_epin(deliveryPincode)
      ? "noth_east"
      : "default";

    // Determine the expected delivery time based on the pickup type and region
    const expectedDeliveryTime =
      deliveryTimeMap[region]?.[pickupType] || "default delivery time";
    ...
    if (
      ...
      if (
        ...
        if (!isMaharastraPincode(pickupPincode)) {
          window.alert(
            "Invalid pickup pin code. Pickup pin code must be from Maharastra."
          );
          return;
        }
        ...
        setCahargablWeight(
          parseFloat(parcelVolume) > parseFloat(parcelWeight)
            ? parcelVolume.toFixed(0)
            : parcelWeight
        );
        setSharedExpressRate(
          parseFloat(volume_Rate) > parseFloat(rate) ? volume_Rate : rate
        );
        setSharedAirRate(
          parseFloat(volume_RateAir) > parseFloat(airRate)
            ? volume_RateAir
            : airRate
        );
  ...
  const handleAirProceed = () => {
    const pickupDetails = {
      pickupPincode,
      pickupPincodeSuggestions,
      pickupSateSuggestions,
    };
    const deliveryDetails = {
      deliveryPincode,
      deliveryPincodeSuggestions,
      deliverySateSuggestions,
    };
    ...
    navigate('/orders/edit/${orderData}',{
      state: {
        pickupDetails,
        deliveryDetails,
        packageDetails,
  ...
  const handleExpressProceed = () => {
    ...
    // navigate(`/orders/edit/${orderData}`, { state: { pickupDetails, deliveryDetails, packageDetails, rate, expectedDeliveryTime } });
  };

  return (
    <div className="grid grid-cols-2">
      {/* Left side: Calculate Rate */}
      <div className="col-span-1 flex justify-center">
        <div className="">
          <p className="text-red-500 text-center">
            {" "}
            {isValidPin ? "" : "Invalid pin code"}
          </p>
          <h4 className="text-center">Calculate Rate</h4>
          <div className="grid p-4 grid-cols-2 gap-4">
            <div className="mb-4">
              ...
              {pickupPincode.length <= 6 ? (
                <p className="text-green-500 font-medium">
                  {" "}
                  {pickupPincodeSuggestions} {pickupSateSuggestions}
                </p>
              ) : (
            ...
            <div className="mb-4">
              ...
              {deliveryPincode.length <= 6 ? (
                <p className="text-green-500 font-medium">
                  {" "}
                  {deliveryPincodeSuggestions} {deliverySateSuggestions}
                </p>
              ) : (
                <p className="text-red-500">{errorMessage}</p>
              )}
          ...
          <div className="">
            <Button variant="twoTone" onClick={calculateRate1}>
              Calculate Rate
            </Button>
          </div>
        </div>
      </div>
      {/* Right side: Shipping Rate */}
          ...
          {!showSpinner && cardsVisible && rate > 0 && airRate > 0 && (
            <div>
              <div>
                <div className="mt-6 ">
                  <button
                    className="lg:text-sm px-6 mb-4 py-2 border rounded-lg bg-red-500 text-white text-lg font-bold"
                ...
                <div className="grid grid-cols-2">
                  ...
                  {pincodeData.length > 0 &&
                    !isMaharastraPincode(deliveryPincode) &&
                    !isgoa_gujratPincode(deliveryPincode) && (
                        ...
                        <div class="bg-white pb-10 shadow-md border rounded-xl">
                          ...
                          <div class=" m-auto">
                            <p class="text-center text-gray-500 pt-5">
                              {" "}
                              {pickupPincode} {pickupPincodeSuggestions} -{" "}
                              {deliveryPincode} {deliveryPincodeSuggestions}{" "}
                            </p>
                            <p class=" text-gray-500 text-center pt-1">
                              {" "}
                              Expected delivery :{" "}
                              {getExpectedDeliveryTime(deliveryPincode, "air")}
                            </p>
                          </div>
                          ...
                          <div className="pt-1 text-gray-500">
                            ...
                            <div className="col-span-1 mt-1 relative inline-block group">
                              ...
                              <span className="tooltip-text text-[10px] absolute bg-red-800 text-white text-center p-2 rounded opacity-0 invisible transition-opacity duration-300 -bottom-10 left-1/2 transform -translate-x-1/2 group-hover:opacity-100 group-hover:visible">
                                if the volumetric weight is greater then actual
                                weight so this will become your chargable weight
                                and volumetric weight is calculated by
                                l*w*h/4750
                              </span>
                            </div>
                          </div>
                          <div className="w-72 lg:w-5/6 m-auto bg-indigo-50 mt-5 p-4 lg:p-4 rounded-2xl">
                            <div className="pt-1">
                              <p className="text-gray-800 text-center font-bold lg:text-xl">
                                ₹{" "}
                                {parseFloat(volume_RateAir) >
                                parseFloat(airRate)
                                  ? volume_RateAir.toFixed(0)
                                  : airRate}
                              </p>
                            </div>
                          </div>
                          ...
                          <div class="bg-blue-700 w-72 lg:w-5/6 m-auto mt-6 p-2 hover:bg-indigo-500 rounded-2xl  text-white text-center shadow-xl shadow-bg-blue-700">
                            <button
                              classs="lg:text-sm text-lg font-bold"
                              onClick={handleAirProceed}
                            >
                              Proceed Now
                            </button>
                          </div>
                  ...
                  {pincodeData.length > 0 &&
                    !isj_kpin(deliveryPincode) &&
                    !isnoth_epin(deliveryPincode) && (
                      <div class="w-80 mt-24 m-auto max-w-sm">
                        <div class="bg-white pb-10 shadow-md border rounded-xl">
                          <h2 class="text-center text-gray-800 text-2xl font-bold pt-6">
                            {" "}
                            Express
                          ...
                          <div class="lg:w-5/6 m-auto bg-indigo-50 mt-5 p-4 lg:p-4 rounded-2xl">
                            <div class="pt-1">
                              <p class="text-gray-800 text-center font-bold lg:text-xl">
                              Amount: ₹ {parseFloat(volume_Rate)>parseFloat(rate) ? volume_Rate.toFixed(0):rate}
                              </p>
                            </div>
                          </div>
                          <div class="bg-orange-400 lg:w-5/6 m-auto mt-6 p-2 hover:bg-orange-500 cursor-pointer rounded-2xl text-white text-center shadow-xl shadow-bg-blue-700">
                            <button
                              classs="lg:text-sm text-lg font-bold"
                              onClick={(e) => handleExpressProceed(rate)}
                            >
                              Update rate
                            </button>
                          </div>
