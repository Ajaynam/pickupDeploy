...
const Documents = (data) => {
    ...
    const orderData = data.data.id
    console.log("calculate", orderData)
    ...
    const handleAirProceed = () => {
        ...
        navigate(`/orders/edit/${orderData}`, { state: { pickupDetails, deliveryDetails, packageDetails, airRate, expectedDeliveryTime } });
    };
    ...
    return (
        <div className=' mt-6 border p-4  '>
            <div className="flex "> <h4 className=" mx-auto ">Calculate Rate</h4> </div>

            <div className="grid grid-cols-2">
                <div className=' mt-4 border p-4 gap-4'>
                    <div className="mb-4">
                        ...
                        {pickupPincode.length <= 6 ? <p className="text-blue-500 text-[13px] font-medium"> {pickupPincodeSuggestions}  {pickupSateSuggestions}</p> : <p className="text-red-500">{errorMessage}</p>}
                    </div>
                    <div className="mb-4">
                        ...
                        />
                        { }
                        {deliveryPincode.length <= 6 ? <p className="text-blue-500 text-[13px] font-medium"> {deliveryPincodeSuggestions}  {deliverySateSuggestions}</p> : <p className="text-red-500">{errorMessage}</p>}
                    </div>
                    <div className="mb-4">
                    ...
                    {!showSpinner && cardsVisible && rate > 0 && airRate > 0 && (
                        <div>
                            <div>
                                <div className=" ">
                                    <svg
                                        className=" px-6 py-2 border w-20 h-10 rounded-lg bg-red-500 text-white text-lg font-bold cursor-pointer"
                                ...
                                <div className='lg:grid lg:grid-cols-2'>
                                    ...
                                    {pincodeData.length > 0 && !isMaharastraPincode(deliveryPincode) && !isgoa_gujratPincode(deliveryPincode) && (
                                        <div class="w-80 mt-2 m-auto lg:mt-16 max-w-sm">
                                    ...
                                    {pincodeData.length > 0 && !isj_kpin(deliveryPincode) && !isnoth_epin(deliveryPincode) && (
                                        ...
                                        <div class=" mt-2 m-auto lg:mt-10 max-w-sm">
                                            ...
                                            <div class=" px-4 pb-10 shadow-md border rounded-xl">
                                                <h2 class="text-center text-gray-800 text-2xl font-bold py-4 "> Express</h2>
                                                <div class=" m-auto">
                                                    <p class="text-center text-gray-500 pt-5">  {pickupPincode} {pickupPincodeSuggestions} -  {deliveryPincode} {deliveryPincodeSuggestions} </p>
                                                    <p class=" text-gray-500 text-center pt-1"> Weight : {parcelWeight} g</p>
                                                ...
                                                <div class="bg-orange-400  lg:w-5/6 m-auto mt-6 p-2 hover:bg-orange-500 cursor-pointer rounded-2xl  text-white text-center shadow-xl shadow-bg-blue-700">
                                                    <button classs="lg:text-sm text-lg font-bold" onClick={() => handleExpressProceed(rate)} > Update rate </button>
                                                </div>
                                            </div>
                                        </div>
                                    )}
