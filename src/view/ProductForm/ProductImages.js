...
import { deleteDeleteProductImageByProductIdAndImageId } from './store/dataSlice'
...
const ImageList = (props) => {
    ...
    const onDelete = async () => {
        await dispatch(deleteDeleteProductImageByProductIdAndImageId(selectedImg))
            ...
            {imgList.map((img) => (
                ...
                >
                    <img
                        ...
                        src={`${appConfig.apiPrefix}${img.img}`}
                        onError={async (e) => {
                            if (e.target.src === `${appConfig.apiPrefix}${img.img}`) {
                                e.target.src = img.img;
                            } else if (e.target.src === img.img) {
            ...
            >
                <div className='flex flex-col items-center overflow-hidden'>
                    ...
                    <img
                        ...
                        src={`${appConfig.apiPrefix}${selectedImg.img}`}
                        onError={async (e) => {
                            if (e.target.src === `${appConfig.apiPrefix}${selectedImg.img}`) {
                                e.target.src = selectedImg.img;
                            } else if (e.target.src === selectedImg.img) {
                                e.target.src = imageError;
                            }
                        }}
