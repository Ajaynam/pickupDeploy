...
import { deleteOrders, getAllSupplier } from '../store/dataSlice'

const OrderDeleteConfirmation = () => {
    const dispatch = useDispatch()
    const selectedRows = useSelector(
        (state) => state.adminShopOwnerList.state
    )
    ...
    const tableData = useSelector(
        (state) => state.adminShopOwnerList.data
    )
    ...
    const deleteSucceed = (success, orders) => {
        if (success) {
            dispatch(getAllSupplier(tableData))
            Toast.push(
                <Notification
